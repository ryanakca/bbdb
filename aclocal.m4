...
AC_DEFUN([BBDB_WITH_VM],
  [ AC_ARG_WITH([vm-dir],
    ...
    if test "x$with_vm_dir" != "x"; then
       BBDB_VM=`${EMACS_PROG} -batch --no-site-file --no-init-file -eval "(push \"${with_vm_dir}\" load-path)" -eval "(if (and (locate-library \"vm-autoload\") (locate-library \"vm-autoloads\")) (message \"vm\"))" 2>&1`
       ...
       case $with_vm_dir in
         /*) # absolute
            ...
            ;;
         *)
            AC_MSG_ERROR([*** emacs requires an absolute path to be passed to `--with-vm-dir'.])
            ;;
       esac
    fi
